...
public class HashAlgorithm implements Comparable<HashAlgorithm> {
     ...
     * java -classpath jacksum.jar net.jacksum.multicore.manyalgos.HashAlgorithm <file>
     */
    static {
...
/*
        ...
        WEIGHTS.put("keccak224", WEIGHTS.get("sha3-224"));
        WEIGHTS.put("keccak-224", WEIGHTS.get("sha3-224"));
        WEIGHTS.put("keccak256", WEIGHTS.get("sha3-256"));
        WEIGHTS.put("keccak-256", WEIGHTS.get("sha3-256"));
        WEIGHTS.put("keccak384", WEIGHTS.get("sha3-384"));
        WEIGHTS.put("keccak-384", WEIGHTS.get("sha3-384"));
        WEIGHTS.put("keccak512", WEIGHTS.get("sha3-512"));
        WEIGHTS.put("keccak-512", WEIGHTS.get("sha3-512"));
        WEIGHTS.put("radiogatun:64", WEIGHTS.get("radiogatun"));
...
*/
    ...
    public static void main(String[] args) {
        ...
        if (file.length() <= 20 * 1024 * 1024) {
            System.err.println("Warning: Filesize of " + file + " is <= 20 MB");
        }
        ...
        if (args.length > 1) {
            ...
            hashes = (Set<String>) JacksumAPI.getAvailableAlgorithms().keySet();
        }
